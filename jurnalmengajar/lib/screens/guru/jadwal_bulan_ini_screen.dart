...
class GuruJadwalBulanIniScreen extends StatefulWidget {
  ...
  const GuruJadwalBulanIniScreen({super.key, this.initialMonth});

  @override
  State<GuruJadwalBulanIniScreen> createState() =>
      _GuruJadwalBulanIniScreenState();
}
  ...
  Widget build(BuildContext context) {
    ...
    final activeSchedulesThisMonth = scheduleProvider.cachedTeacherSchedules
        .where((s) {
          ...
          return s.date.year == _currentMonth.year &&
    ...
    final filteredSchedules =
        groupedMonthSchedules.where((group) {
          final j = _findMatchingJournal(
            group,
          ...
          if (_searchQuery.isNotEmpty) {
            ...
            final matchesClass = cls.name.toLowerCase().contains(
              _searchQuery.toLowerCase(),
            );
            final matchesSubject = subject.name.toLowerCase().contains(
              _searchQuery.toLowerCase(),
            );
    ...
    if (isFilled) {
      if (matchingJournal.isTeacherAbsence) {
        if (matchingJournal.status == 'verified') {
          ...
          statusTitle = matchingJournal.isTeacherSick
              ? 'Sakit (Disetujui)'
              : 'Izin (Disetujui)';
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // Left content: Date, Subject, Class & Hours
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
            ...
            SizedBox(width: 10.w),
            // Right content: Status badge at top, and Action button directly below it!
            Column(
